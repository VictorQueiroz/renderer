!function(global) {
    ...
    function values(object) {
        var i, keys = Object.keys(object), ii = keys.length, values = [];
        for (i = 0; ii > i; i++) values[i] = object[keys[i]];
        return values;
    }
    ...
    function Interpolate(text, startSymbol, endSymbol) {
        ...
        for (var exp, endIndex, startIndex, text = this.text, index = this.index, concat = [], endSymbol = this.endSymbol, startSymbol = this.startSymbol, endSymbolLength = endSymbol.length, startSymbolLength = startSymbol.length, expressionPositions = []; index < text.length; ) {
    ...
    renderer.parse = function(exp, cache) {
        if (expsCache.hasOwnProperty(exp) && 0 != cache) return expsCache[exp];
    ...
    extend(renderer, {
        ...
        bootstrap: function(element) {
            ...
            args.push(rootScope);
            for (var i = 0; i < bootstrapArgs.length; i++) args.push(bootstrapArgs[i]);
    ...
    inherits(Scope, Watcher, {
        ...
        add: function(name, restrict, startAttrName, endAttrName) {
            ...
            for (i = 0; ii > i; i++) directive = directives[i], -1 !== directive.restrict.indexOf(restrict) && (!isDefined(maxPriority) || directive.priority > maxPriority) ? (startAttrName && (directive = inherit(directive, {
                $$start: startAttrName,
                $$end: endAttrName
            })), this.directives.push(directive)) : (maxPriority = void 0, delete this.maxPriority);
        },
    ...
    forEach(Token, function(value, key) {
        ...
        parseTemplateLiteral: function(ast, id, recursion) {
            var i, expression, id, quasi, template = [], expressions = [];
