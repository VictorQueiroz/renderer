!function(global) {
    ...
    function compile(node, transcludeFn, maxPriority) {
        function publicLinkFn(scope, cloneConnectFn) {
            var linkNodes;
            if (cloneConnectFn) {
                for (var clones = new Array(nodes.length), i = 0; i < clones.length; i++) clones[i] = nodes[i].cloneNode(1);
                linkNodes = clones;
            } else linkNodes = nodes;
            return cloneConnectFn && cloneConnectFn(linkNodes, scope), compositeLinkFn && compositeLinkFn(scope, linkNodes), 
            linkNodes;
        }
        var nodes;
        isArray(node) || node instanceof NodeList == !0 ? (nodes = node, node = null) : (nodes = [], 
        nodes.push(node));
    ...
    function replaceWith(node, replaceElement) {
        ...
        parent && parent.replaceChild(replaceElement, node);
    }
    function data(node, key, value) {
        if (node && node.nodeType === Node.ELEMENT_NODE) {
            var id;
            node.hasOwnProperty(cacheKey) ? id = node[cacheKey] : (id = node[cacheKey] = nextId(), 
            elCache[id] = {});
            var cache = elCache[id];
            if (!key) return cache;
            if (!value && cache.hasOwnProperty(key)) return cache[key];
    ...
    function inheritedData(element, name, value) {
        ...
        for (var names = isArray(name) ? name : [ name ]; element; ) {
            for (var i = 0, ii = names.length; ii > i; i++) if (value = data(element, names[i])) return value;
            element = element.parentNode || element.nodeType === Node.DOCUMENT_FRAGMENT_NODE && element.host;
        }
    }
    function directiveIsMultiElement(name) {
        if (registry.hasOwnProperty(name)) {
            var i, ii, directives = registry.$$get(name);
            for (i = 0, ii = directives.length; ii > i; i++) if (directives[i].multiElement) return !0;
        }
        return !1;
    }
    function groupElementsLinkFnWrapper(linkFn, attrStart, attrEnd) {
        return function(scope, element, attrs, controllers, transcludeFn) {
            return element = groupScan(element, attrStart, attrEnd), linkFn(scope, element, attrs, controllers, transcludeFn);
        };
    }
    function groupScan(node, attrStart, attrEnd) {
        ...
        if (attrStart && node && node.nodeType == Node.ELEMENT_NODE && node.hasAttribute(attrStart)) {
            do {
                if (!node) throw new Error("Unterminated attribute, found '" + attrStart + "' but no matching '" + attrEnd + "' found.");
    ...
    function apply(directives, node, attributes, transcludeFn) {
        function addLinkFn(pre, post, attrStart, attrEnd) {
            ...
            pre.require = require, pre.newScopeType = isDefined(directive.scope), pre.directiveName = directiveName, 
        ...
        function nodeLinkFn(scope, node, childLinkFn, transcludeFn) {
            ...
            switch (nodeLinkFn.scopeType) {
              ...
              case SCOPE_ISOLATED:
                newScope = scope.clone(!0), directiveBindings(scope, newScope, nodeLinkFn.bindings, attributes);
                break;
        ...
        var i, ii, linkFn, directive, directiveName, directiveValue, template, attrStart, attrEnd, terminalPriority = -Number.MAX_VALUE, childTranscludeFn = transcludeFn, directiveControllers = Object.create(null), preLinkFns = [], postLinkFns = [];
        for (i = 0, ii = directives.length; ii > i && (directive = directives[i], directiveName = directive.name, 
        attrStart = directive.$$start, attrEnd = directive.$$end, attrStart && (node = groupScan(node, attrStart, attrEnd)), 
        !(terminalPriority > directive.priority)); i++) {
            if (directiveValue = directive.scope) if (isObject(directiveValue)) {
                if (nodeLinkFn.scope) throw new Error("You can't define a new isolated scope on a node that already has a child scope defined");
                nodeLinkFn.bindings = directiveValue, nodeLinkFn.scopeType = SCOPE_ISOLATED;
            } else nodeLinkFn.scopeType = SCOPE_CHILD;
            ...
            (directiveValue = directive.template) ? node.innerHTML = directiveValue : directive.transclude && "element" !== directive.transclude && (node.innerHTML = ""), 
            directive.controller && (directiveControllers[directive.name] = directive), linkFn = directive.compile(node, attributes, childTranscludeFn), 
            isFunction(linkFn) ? addLinkFn(0, linkFn, attrStart, attrEnd) : isObject(linkFn) && addLinkFn(linkFn.pre, linkFn.post, attrStart, attrEnd), 
            directive.terminal && (nodeLinkFn.terminal = !0, terminalPriority = Math.max(terminalPriority, directive.priority));
        }
        return nodeLinkFn.transcludeFn = childTranscludeFn, nodeLinkFn.controllers = Object.create(null), 
        nodeLinkFn;
    }
        ...
        forEach(bindings, function(mode, key) {
            ...
            mode) {
              case "@":
                ...
                }), isString(attrs[attrName])) {
                    var interpolateFn = interpolate(attrs[attrName]);
                    interpolateFn && (dest[key] = interpolateFn(scope));
                }
                break;

              case "=":
                if (!attrs.hasOwnProperty(attrName)) break;
                parentGet = parse(attrs[attrName]), parentSet = parentGet.assign, lastValue = dest[key] = parentGet(scope);
    ...
    function getControllers(require, node, controllers, directiveName) {
        ...
        if (isArray(require)) {
            ...
            var i, ii;
            for (i = 0, ii = value.length; ii > i; i++) value[i] = getControllers(require[i], node, controllers, directiveName);
            return value;
        }
        ...
        if ("^^" === inheritType ? node = node.parentNode : value = controllers && controllers[name], 
        !value) {
    ...
    function addDirective(name, type, directives, maxPriority, startAttrName, endAttrName) {
        var i, ii, instances, directive;
        if (instances = registry.$$get(name)) for (i = 0, ii = instances.length; ii > i; i++) directive = instances[i], 
        (isUndefined(maxPriority) || maxPriority > directive.priority) && directive.type.indexOf(type) > -1 && (startAttrName && (directive = inherit(directive, {
            $$start: startAttrName,
            $$end: endAttrName
            ...
            compile: function() {
                return {
                    pre: function(scope, element, attrs) {
                        attrs[name] = interpolateFn(scope), scope.watchGroup(interpolateFn.expressions, function() {
    ...
    function scan(node, directives, attributes, maxPriority) {
        var i, ii, name, attr, attrs, value, classes;
        switch (node.nodeType) {
          case Node.ELEMENT_NODE:
            name = camelCase(node.tagName), attrs = node.attributes, addDirective(name, "E", directives, maxPriority);
            var nodeAttrName, attrStartName, attrEndName;
            for (i = 0, ii = attrs.length; ii > i; i++) {
                attr = attrs[i], value = trim(attr.value), nodeAttrName = attr.name, attrStartName = !1, 
                attrEndName = !1;
                var multiElementMatch = camelCase(nodeAttrName).match(MULTI_ELEMENT_DIR_RE);
                multiElementMatch && directiveIsMultiElement(multiElementMatch[1]) && (attrStartName = nodeAttrName, 
                attrEndName = nodeAttrName.substr(0, nodeAttrName.length - 5) + "end", nodeAttrName = nodeAttrName.substr(0, nodeAttrName.length - 6)), 
                name = camelCase(nodeAttrName), attributes[name] = value, addAttrInterpolateDirective(name, value, directives), 
                addDirective(name, "A", directives, maxPriority, attrStartName, attrEndName);
            }
            for (classes = node.classList, i = 0, ii = classes.length; ii > i; i++) name = camelCase(classes[i]), 
    ...
    function compileNodes(nodeList, transcludeFn, maxPriority) {
        ...
        for (i = 0; i < nodeList.length; i++) directives = [], attributes = new Attributes(nodeList[i]), 
        scan(nodeList[i], directives, attributes, 0 === i ? maxPriority : void 0), nodeLinkFn = directives.length ? apply(directives, nodeList[i], attributes, transcludeFn) : null, 
        childLinkFn = !(nodeLinkFn && nodeLinkFn.terminal) && nodeList[i] && nodeList[i].childNodes && nodeList[i].childNodes.length, 
        ...
        (childLinkFn || nodeLinkFn) && (linkFnFound = !0, nodeLinkFnFound = nodeLinkFnFound || nodeLinkFn, 
    ...
    forEach(Token, function(value, key) {
        ...
        parseCallExpression: function(ast, id, recursion) {
            ...
            right = this.nextId(), left = ast.callee, this.recurse(ast.callee, right, left, function() {
                grammar.ifNotNull(right, function() {
                    forEach(ast.arguments, function(expr) {
                        self.recurse(expr, self.nextId(), void 0, function(arg) {
                            args.push(arg);
                        });
                    }), expression = left.name ? this.member(left.context, left.name, left.computed) + this.block(args.join(",")) : this.execute(right, args), 
